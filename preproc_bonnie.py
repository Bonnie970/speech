...
def main():
	...
	for word, label in labels.items():
		...
		for i,fn in enumerate(files):
			...
			silence_free = np.array([x for x in samples if ((x<-50) or (x>50))])
			# modify to original size by adding zeros to the end
			samples = np.append(silence_free, np.zeros(len(samples) - len(silence_free)))
